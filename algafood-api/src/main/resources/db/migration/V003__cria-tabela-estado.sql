...
insert into algafood.estado (nome)
 select distinct nome_estado from algafood.cidade;

alter table algafood.cidade add column estado_id bigint not null;

update algafood.cidade as c set c.estado_id  =
(select e.id from algafood.estado as e where e.nome = c.nome_estado);
